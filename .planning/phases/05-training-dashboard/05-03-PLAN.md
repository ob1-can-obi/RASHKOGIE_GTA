...
Key design points:
- All SQL uses parameterized queries (?, not f-string) per T-05-02
...
- PUT /params writes to file (D-08) AND broadcasts to training scripts (D-06)
  ...
  <acceptance_criteria>
    ...
    - dashboard/routes/metrics.py contains `async def get_metrics`
    - dashboard/routes/metrics.py contains `async def get_latest_metrics`
    ...
    - dashboard/routes/metrics.py uses parameterized SQL queries only (? placeholders)
  ...
  <done>Metrics and params REST routes created with parameterized SQL, validated input models, config file locking, and WebSocket broadcast on param changes.</done>
</task>
...
```
  ...
  <done>Server wired with collector background task, WebSocket endpoints for browser and training clients, REST routes for metrics and params, all previously-skipped tests now pass with real assertions.</done>
</task>
...
<success_criteria>
Server starts with collector background task, WebSocket endpoints for browser (/ws/browser) and training script (/ws/train) clients, REST endpoints for metrics (GET /api/metrics, /api/metrics/latest, /api/metrics/decisions, /api/pipeline/status) and params (GET/PUT /api/params) with validation and file persistence. All dashboard tests pass.
</success_criteria>
