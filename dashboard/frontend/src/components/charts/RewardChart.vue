...
const chartData = computed(() => ({
  labels: store.steps,
  datasets: [{ label: 'Reward', data: store.rewards, borderColor: '#10b981', borderWidth: 1.5, pointRadius: 0, fill: false }]
}))
const options = { scales: { x: { title: { display: true, text: 'Step' } }, y: { title: { display: true, text: 'Reward' } } } }
</script>
